...
ARM_int_lock_SYS:
    ...
    MSR     cpsr_c, #(SYS_MODE | NO_INT)    /* disable IRQ/FIQ */
    BX      lr                      /* return to the original CPSR in r0 */
...
/* IRQ Entry */
 ...
 /* end IRQ Entry */
    ...
    /* NOTE: BSP_irq might re-enable IRQ interrutps (the FIQ is enabled
    * already), if IRQs are prioritized by an interrupt controller. */
    LDR     r12, =C_irq
    MOV     lr, pc                  /* copy the return address to tink register */
    BX      r12                     /* call the C IRQ-handler (ARM/THUMB) */
    
/* IRQ Exit */
    MSR     cpsr_c, #(SYS_MODE | NO_INT)    /* SYSTEM mode, IRQ/FIQ disabled */
    MOV     r0, sp                  /* maake sp_SYS visible to IRQ mode */
    ...
    NOP                             /* can't access banked regs immediately */
...
/* FIQ Entry */
    ...
    SUB     r0, lr, #4      /* put return aaddress in r0_SYS */
...
/* end FIQ entry */
    ...
    /* NOTE: BSP_fiq must NEVER enable IRQ/FIQ interrupts! */
    LDR     r12, =C_fiq
...
/* FIQ exit */                      /* both IRQ/FIQ disabled (see NOTE above) */
    ...
    LDMFD   sp, {r0-r3,r12,lr}^     /* unstall all saved USER/SYSTEM registers */
    NOP                             /* can't access banked regs immediately */
    LDR     lr, [sp, #(6*4)]        /* load return address from the SYS stack */
    MOVS    pc, lr                  /* return restoring CPSR from SPSR */
/* end FIQ exit */
...
Csting_swi:     .string "Software Interrupt"
